...
import com.example.crptocurrencypl.domain.use_case.get_coin.GetCoinUseCase
import com.example.crptocurrencypl.domain.use_case.get_coins.GetCoinsUseCase
    ...
    private fun getCoin(coinId : String){
        getCoinUseCase(coinId).onEach { result ->
            when (result){
                ...
                is Resource.Error -> {
                    _state.value = CoinDetailState(error = result.message ?: "fdgfdgfd")
                }
                is Resource.Loading -> {
